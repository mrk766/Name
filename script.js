document.addEventListener('DOMContentLoaded', () => {
    const state = {
        currentView: 'chatroom', // chatroom | coderoom | singlePost
        activePostId: null,
    ...
    const navLinks = {
        ...
        coderoom: document.getElementById('nav-posts'),
    };
    ...
    const ensureUsername = () => {
        if (!state.username) {
            const name = prompt("Enter your name (one-time):");
            if (name) {
    ...
    const renderChatroom = () => {
        ...
        feed.innerHTML = combined.map(item => {
            if (item.type === 'message') return `<div class="chat-message me">${escapeHtml(item.text)}</div>`;
            ...
            if (item.type === 'comment') {
                ...
                return `<div class="comment-in-chat"><div class="reply-quote">Reply to "${parent?.title || 'a post'}"</div>${escapeHtml(item.text)}</div>`;
            }
        }).join('');
    ...
    const renderCoderoom = () => {
        ...
        const postGrid = document.getElementById('coderoom-post-grid');
        ...
        const filteredPosts = state.posts.filter(p => !state.selectedSubject || p.subject.trim().toLowerCase() === state.selectedSubject);

        const sorted = [...filteredPosts].sort((a, b) => {
            if (postSort.value === 'oldest') return new Date(a.timestamp) - new Date(b.timestamp);
            if (postSort.value === 'newest') return new Date(b.timestamp) - new Date(a.timestamp);
    ...
    const setupEventListeners = () => {
        ...
        document.getElementById('create-post-btn').addEventListener('click', () => {
            ...
            const subInput = document.getElementById('post-subject');
        ...
        // Delegate all clicks on post cards and chat buttons
